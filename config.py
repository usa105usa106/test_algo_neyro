...
APP_VERSION = "67_full_GMAIL_ROUTER_NONBLOCKING_START"
...
def _resolve_gmail_public_base_url() -> str:
    ...
    v67 exposes a stable nginx router on container port 80 and prefers
    Coolify's explicit port-specific ``SERVICE_URL_<ID>_80`` route. The external URL still uses
    normal HTTPS without a visible ``:80``; the suffix only tells Coolify which
    internal container port Traefik must use.
    """
    for key in (
        "GMAIL_PUBLIC_BASE_URL",
        "GMAIL_REDIRECT_URI",  # backward compatibility; endpoint is stripped
        # Prefer an explicit port-specific Coolify magic URL. It creates a
        # deterministic proxy route to the actual listener and avoids stale or
        # ambiguous generic SERVICE_URL values after redeploy/domain changes.
        "SERVICE_URL_GMAIL-AUTH_80",
        "SERVICE_URL_GMAILAUTH_80",
        "SERVICE_FQDN_GMAIL-AUTH_80",
        "SERVICE_FQDN_GMAILAUTH_80",
        # Generic values remain as backward-compatible fallbacks.
