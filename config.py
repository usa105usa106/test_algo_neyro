...
def _resolve_gmail_redirect_uri() -> str:
    """Resolve the public OAuth callback without requiring a manual redirect variable.

    Priority:
    1. Explicit GMAIL_REDIRECT_URI (backward compatibility).
    2. Coolify magic URL generated by SERVICE_URL_GMAIL-AUTH_8080.
    3. Optional GMAIL_PUBLIC_BASE_URL.
    4. Coolify predefined application URL.
    """
    ...
    for key in (
        ...
        if not raw:
            continue
        # COOLIFY_URL may contain several comma-separated URLs.
        first = raw.split(",", 1)[0].strip()
        if first:
            return _normalize_callback_url(first)
    return ""
...
def load_settings() -> Settings:
    ...
    settings = Settings(
        ...
        gmail_oauth_listen_port=int(os.getenv("GMAIL_OAUTH_LISTEN_PORT", "8080")),
