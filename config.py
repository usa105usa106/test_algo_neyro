...
def _resolve_gmail_public_base_url() -> str:
    ...
    v70 supports both Coolify Dockerfile and Docker Compose deployments.
    The same bot container serves Telegram polling and the Gmail callback on
    container port 80, so the proxy target always matches the listening port.
    """
    for key in (
        "GMAIL_PUBLIC_BASE_URL",
        "GMAIL_REDIRECT_URI",  # backward compatibility; endpoint is stripped
        # v70 direct single-container route identifier.
