...
- Quality 68-69 is allowed as cautious Intraday A LIMIT when report.decision=MANUAL_REVIEW, DATA_WARNING is absent, trap/late are low, pressure supports direction, report already states a CLOSED hold/reclaim/rejection, and RR is clean. Do not demand an extra 15m close/retest beyond the confirmation already present in report/CSV.
...
def build_intraday_candidates_archive(
    ...
    manifest = {
        ...
        "answer_rule_for_chatgpt": "Use only this archive data. Brief Russian answer. report.decision=MANUAL_REVIEW is a candidate for human check, not automatic permission, but do not over-confirm it. Maximum one real setup per archive, Intraday A only; B/B+/A- => WAIT observation only. LIMIT only. Bold Entry/Stop/TP numbers. Use midpoint for ranges. Require CLOSED 1m/15m rejection/hold or sweep reclaim; unfinished candles are context only; reject strong opposite 4H structure. If report already states a closed hold/reclaim/rejection and CSV confirms it, do not require an extra 15m close/retest beyond the report. If price moved toward TP before entry, do not chase, but a return to the planned retest/entry zone is allowed if closed 15m structure remains valid. If rank_score/quality_score < 68, default WAIT; 68-69 may be cautious Intraday A LIMIT when DATA_WARNING is absent, trap/late are low, pressure supports direction, closed confirmation exists, and RR is clean. Near 24h high/low is not automatic reject; it can be TP/liquidity target if TP1/RR room is enough and there is no closed rejection/sweep against the trade. SL must be structural beyond actual setup invalidation / nearest liquidity magnet; do not automatically force far 24h high/low. If nearest day edge is too close or structural SL ruins RR, answer WAIT. TP1 closes 33%; move remainder to BE only after 15m close beyond TP1 in trade direction or after TP2.",
